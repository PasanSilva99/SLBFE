...
namespace SLBFE.Models
{
...

    {
        ...
        public static void InitializeDatabase()
        {
            ...
            using (SQLiteConnection con = new SQLiteConnection($"Data Source={DatabasePath}; Version=3;"))
            {
                try
                {
                    ...
                    "CREATE TABLE IF NOT EXISTS " +
                    "FeedbackReply (" +
                        "ReplyID TEXT, " +  // Autogenerated Unique ID
        ...
        /// <returns>0 - Failed, 1 - Successs, -1 - Error in DB, -2 Error in Object or Connection</returns>
        internal static int NewComplaint(Complaint complaint)
        {
            using (SQLiteConnection con = new SQLiteConnection($"Data Source={DatabasePath}; Version=3;"))
            {
                try
                {
                    ...
                    insertCommand.Parameters.AddWithValue("@id", complaint.ComplaintID);
                    insertCommand.Parameters.AddWithValue("@email", complaint.Email);
                    insertCommand.Parameters.AddWithValue("@userame", complaint.Username);
                    insertCommand.Parameters.AddWithValue("@iscomplaint", complaint.isComplaint ? 1 : 0); // is complaint is bool but the database has int
                    insertCommand.Parameters.AddWithValue("@companyId", complaint.CompanyID);
                    insertCommand.Parameters.AddWithValue("@companyName", complaint.CompanyName);                    
                    insertCommand.Parameters.AddWithValue("@sentDate", complaint.SentDate.ToString("G"));  // save the data as General Date time
                    insertCommand.Parameters.AddWithValue("@content", complaint.Content);

                    var affRows = insertCommand.ExecuteNonQuery();

                    if (affRows > 0)  // if it is an successfull registration
                    {
                        Log($"Successfully Saved Complaint ({complaint.ComplaintID}) From {complaint.Email}");
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    // Log the error to the API log
                    Log("Saving Complaint Failed (Database)");
                    Log(ex.ToString());
                }
            }

            // Log the error to the API log
            Log("Uncaught Error on Saving Complaint");
            return -2;
        }
        ...
        internal static int ReplyComplaint(ComplaintReply reply)
        {
            using (SQLiteConnection con = new SQLiteConnection($"Data Source={DatabasePath}; Version=3;"))
            {
                try
                {
                    ...
                    insertCommand.Parameters.AddWithValue("@sentDate", reply.SentDate.ToString("G"));  // save the data as General Date time
                    insertCommand.Parameters.AddWithValue("@content", reply.Content);

                    var affRows = insertCommand.ExecuteNonQuery();

                    if (affRows > 0)  // if it is an successfull Save
                    {
                        Log($"Successfully Saved Reply for ({reply.FeedbackID}) From {reply.Email}");
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    // Log the error to the API log
                    Log("Saving Complaint Reply Failed (Database)");
                    Log(ex.ToString());
                }
            }

            // Log the error to the API log
            Log("Uncaught Error on Saving Complaint Reply");
            return -2;
        }

        internal static int DeleteComplaint(string complaintID)
        {
            using (SQLiteConnection con = new SQLiteConnection($"Data Source={DatabasePath}; Version=3;"))
            {
                try
                {
                    con.Open();
                    SQLiteCommand insertCommand = new SQLiteCommand();
                    insertCommand.CommandText = "DELETE FROM " +
                        "Feedback " +
                            "WHERE " +
                                "ID=@id ";
                    ...
                    insertCommand.Parameters.AddWithValue("@id", complaintID);

                    var affRows = insertCommand.ExecuteNonQuery();

                    if (affRows > 0)  // if it is an successfull registration
                    {
                        Log($"Successfully Deleted Complaint {complaintID}");
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    // Log the error to the API log
                    Log("Deletion Complaint Failed (Database)");
                    Log(ex.ToString());
                }
            }

            // Log the error to the API log
            Log("Uncaught Error on Deleting Complaint");
            return -2;
        }

        internal static int DeleteComplaintReply(string feedbackID, string replyID)
        {
            using (SQLiteConnection con = new SQLiteConnection($"Data Source={DatabasePath}; Version=3;"))
            {
                try
                {
                    ...
                    insertCommand.Parameters.AddWithValue("@id", replyID);
                    insertCommand.Parameters.AddWithValue("@feedbackid", feedbackID);

                    var affRows = insertCommand.ExecuteNonQuery();

                    if (affRows > 0)  // if it is an successfull registration
                    {
                        Log($"Successfully Deleted Complaint Reply {replyID}");
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    // Log the error to the API log
                    Log("Deletion Complaint Reply Failed (Database)");
                    Log(ex.ToString());
                }
            }

            // Log the error to the API log
            Log("Uncaught Error on Deleting Complaint Reply");
            return -2;
        }
